"""Gateway: client-facing edge of the tap-train demo.

Accepts a plain `TrainRequest` JSON from the client (all fields optional —
defaults come from the TrainRequest model), wraps it in a signed envelope with
a monotonic id, relays to the Tap, verifies the response envelope, unwraps the
inner `TrainResponse`, and returns it as plain JSON.

The wait is intentionally long (10 minutes) because training a 32-step LoRA
adapter takes minutes; the Gateway is the synchronous head of the chain.
"""
...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            req = TrainRequest.model_validate(body)
        ...
        try:
            ...
            outbound = Request(
                f"{self.tap_url}/train",
                data=data,
                headers={"Content-Type": "application/json"},
                method="POST",
            )
            with urlopen(outbound, timeout=600) as resp:
        ...
        try:
            inner = TrainResponse.model_validate(signed_resp.data.payload)
        except Exception as exc:  # noqa: BLE001
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Tap-train Gateway")
