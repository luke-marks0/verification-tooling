"""Tap: relays SignedEnvelope<TrainRequest> from Gateway to Host Cluster.
...
logged but do not propagate -- verification is async. Timeouts are generous
(10 min) because /verify itself re-trains.
"""
...
class TapHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            outbound = Request(
                f"{self.host_url}/train",
                data=json.dumps(req_env.model_dump()).encode("utf-8"),
                headers={"Content-Type": "application/json"},
                method="POST",
            )
            # 10-minute synchronous training budget on the Host path.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Tap-train Tap")
