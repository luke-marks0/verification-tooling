...
python3 modules/inference/runner/main.py --mode mock --manifest "$MANIFEST" --lockfile "$BUILT" --out-dir "$RUN_A" --replica-id replica-0
python3 modules/inference/runner/main.py --mode mock --manifest "$MANIFEST" --lockfile "$BUILT" --out-dir "$RUN_B" --replica-id replica-0
...
if python3 modules/inference/runner/main.py --mode mock --manifest "$MANIFEST" --lockfile "$BUILT" --out-dir "$TMP_DIR/run-strict-bad" --runtime-hardware "$RUNTIME_HW"; then
...
python3 modules/inference/runner/main.py --mode mock --manifest "$NONSTRICT_MANIFEST" --lockfile "$TMP_DIR/built.nonstrict.lock.json" --out-dir "$RUN_C" --runtime-hardware "$RUNTIME_HW"
